...
from models.DualConvLayer import SpatialConvLayer, TemporalConvLayer
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, mask: Tensor = None) -> Tensor:
        ...
        queries = self.queries(query).view(b, n, h, -1).permute(0, 2, 1, 3)
        # keys: (b, n, h*d) -> (b, h, n, d)
        keys = self.keys(key).view(b, n, h, -1).permute(0, 2, 1, 3)
        # values: (b, n, h*d) -> (b, h, n, d)
        values = self.values(value).view(b, n, h, -1).permute(0, 2, 1, 3)
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, mask: Tensor = None) -> Tensor:
        ...
        queries = self.queries(query).view(b, n, h, -1).permute(0, 2, 1, 3)
        # keys: (b, n, h*d) -> (b, h, n, d)
        keys = self.keys(key).view(b, n, h, -1).permute(0, 2, 1, 3) # (B,num_head=4,61,16)
        # values: (b, n, h*d) -> (b, h, n, d)
        values = self.values(value).view(b, n, h, -1).permute(0, 2, 1, 3)
        energy = torch.einsum('bhqd, bhkd -> bhqk', queries, keys) # (B,num_head=4,61,61)
        if mask is not None:
            fill_value = torch.finfo(torch.float32).min
            energy.mask_fill(~mask, fill_value)

        self.attention_weights = F.softmax(energy / self.scaling, dim=-1)
        att = self.att_drop(self.attention_weights) # (B,num_head=4,61,61)
        out = torch.einsum('bhal, bhlv -> bhav ', att, values) # (B,num_head=4,61,16)
        # out = rearrange(out, "b h n d -> b n (h d)")
...
class ClassificationHead(nn.Module):
    def __init__(self,args, num_classes, num_domains, emb_size, domain_generalization, dropout):
    ...
    def forward(self, out):
        ...
        if self.args.modality == "EEG":
            out = torch.cat([eeg_temporal, eeg_temporal, eeg_spatial, eeg_spatial], dim=-1)
        elif self.args.modality == "fNIRS":
            out = torch.cat([nirs_temporal, nirs_temporal, nirs_spatial, nirs_spatial], dim=-1)
        elif self.args.modality == "both":
            out = torch.cat([eeg_temporal, nirs_temporal, eeg_spatial, nirs_spatial], dim=-1)
        out = self.fusion(out)
...
class HybridTransformer(nn.Module):
    def __init__(self, args,final_activation=None, depth=2, query_size=64, key_size=64, value_size=64, emb_size=64, num_heads=4, expansion=2, domain_generalization=0,
                 conv_dropout=0.3, self_dropout=0.3, cross_dropout=0.3, cls_dropout=0.3, num_classes=3, num_domains=0, device=None):
        super().__init__()
        ...
        with torch.no_grad():
            eeg, nirs = torch.randn(1, 64, 200), torch.randn(1, 51, 25)
        ...
        self.classify = ClassificationHead(args,num_classes, num_domains, emb_size, domain_generalization, cls_dropout)
    ...
    def forward(self, eeg, nirs):
        ...
        # print(temporal_eeg.shape, temporal_nirs.shape)# torch.Size([32, 10, 64]) torch.Size([32, 6, 64])
        # print(spatial_eeg.shape, spatial_nirs.shape)# torch.Size([32, 60, 64]) torch.Size([32, 40, 64])
